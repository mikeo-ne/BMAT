...
/**
 * Reusable airplay table.
 *
 * Extracted from the artist portal so the same "spins across the Uganda FM
 * panel, split by region" view can be dropped into the charts and royalty pages.
 * Every piece of chrome is optional: the region switcher, the search box and the
 * remove action can each be turned off when the parent drives that state.
 */
export interface AirplayTableProps {
  tracks: Track[];
  /** Which region the spin and station columns report on. */
  focusRegion?: Region | "All";
  /** Omit to render a read-only table with no region switcher. */
  ...
  emptyHint?: string;
  showSearch?: boolean;
  /** Defaults to true whenever an onFocusRegion handler is supplied. */
  showRegionFilter?: boolean;
}

export function AirplayTable({
  tracks,
  focusRegion = "All",
  onFocusRegion,
  onDelete,
  ...
  emptyHint = "No track matches that search.",
  showSearch = true,
  showRegionFilter = Boolean(onFocusRegion),
}: AirplayTableProps) {
  ...
  const columns: { key: SortKey | null; label: string; align?: "right" }[] = [
    ...
    ...(onDelete ? [{ key: null, label: "", align: "right" as const }] : []),
  ];

  return (
    <section className="panel overflow-hidden" aria-labelledby="airplay-table-heading">
      <header className="flex flex-wrap items-end justify-between gap-3 border-b border-line px-4 py-3.5 sm:px-5">
        <div>
          <h2 id="airplay-table-heading" className="text-sm font-semibold tracking-tight">
            {title}
          </h2>
                  ...
                  >
                    <td className="px-3 py-3 pl-5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="min-w-0">
                          ...
                          {track.genre && (
                            <span className="mt-0.5 inline-block rounded border border-line px-1 text-[9px] uppercase tracking-wider text-muted">
                              {track.genre}
                            </span>
                          )}
                        </span>
                    ...
                    <td className="px-3 py-3">
                      <Sparkline
                        values={trend}
                        stroke={
                          focusRegion === "All" ? "var(--brand)" : REGION_META[focusRegion].accent
                        }
                        label={`${track.title} spins over 14 days`}
                      />
                    </td>

                    <td className={["px-3 py-3", onDelete ? "pr-2" : "pr-5"].join(" ")}>
                      <div className="w-40">
            ...
            <tfoot>
              <tr className="border-t border-line bg-surface-2/60 text-xs text-muted">
                ...
                <td
                  ...
                  className={["px-3 py-2.5 text-right", onDelete ? "pr-5" : "pr-5"].join(" ")}
                >
